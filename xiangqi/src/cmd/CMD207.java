	...
	public ChannelBuffer getBytes(int cmd, ChannelBuffer data) {
		...
		if ("发起团购".equals(name)) {// 新开团
			...
			if (tuan.getPeoples() == 0) {
				...
				tuan.setChannel(channel);
				tuan.setRegChannel(channel + "#");
				tuan.setNumber(number);
				tuan.setTitle(goods_name);
				tuan.setInfo(goods_note);
				tuan.setVersion(device.getVersion());
				// 团购时间以收到支付结果为准。
			...
			} else if (ServerTimer.distOfSecond() > tuan.getLastTime()) {
			...
			if(device.getState() != 1){
				return goBackBuffer(buf, 2, "您未参与团购");
			}			
			Tuan tuan = Tuan.getByImei(device.getImei());
			if(tuan == null){//非自建团
				Mobile mobile = Mobile.getByImei(device.getImei());
				if(mobile == null || mobile.getTuanID() == 0){
					return goBackBuffer(buf, 2, "您未参与团购");
				}else{
			...
			if(tuan == null){
				return goBackBuffer(buf, 2, "您未参与团购");
			}else if (tuan.getPeoples() == 0) {
				return goBackBuffer(buf, 2, "您还未成功建团");
